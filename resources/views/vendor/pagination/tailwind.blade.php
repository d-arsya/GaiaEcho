            ...
            @if (!$paginator->onFirstPage())
                    ...
                    @foreach ($elements as $element)
                        ...
                        <span aria-current="page">
                                        <span class="bg-mainGreen relative inline-flex items-center px-4 py-2 -ml-px text-sm font-semibold text-white border border-gray-300 cursor-default leading-5 dark:bg-gray-800 dark:border-gray-600">{{ $page }}</span>
                                    </span>
                                    @elseif($page == $paginator->currentPage()-1 || $page == $paginator->currentPage()+1)
                                    <a href="{{ $url }}" class="relative hover:bg-mainGreen hover:text-white inline-flex items-center px-4 py-2 -ml-px text-sm font-medium bg-white border border-gray-300 leading-5 focus:z-10 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150 dark:bg-gray-800 dark:border-gray-600 dark:text-gray-400 dark:hover:text-gray-300 dark:active:bg-gray-700 dark:focus:border-blue-800" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
