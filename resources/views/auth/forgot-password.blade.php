        ...
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto h-screen lg:py-0">
            <div class="w-full bg-white rounded-2xl max-w-md">
                <div class="p-10">
                    ...
                    @error('success')
                    <div class="p-2 bg-mainGreen rounded-lg w-full text-sm text-white mt-3 italic">{{ $message }}</div>
                    @enderror
                    @error('failed')
                    ...
                    <form method="POST" action="{{ route('password.email') }}">
                        ...
                        <button type="submit" class="mt-6 w-full text-white bg-mainGreen font-bold rounded-full text-lg px-5 py-2 text-center">Kirim</button>
