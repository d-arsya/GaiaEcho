...
<body class="bg-slate-100 overflow-y-hidden">
  <header class="w-full py-5 px-5 z-50">
    <div class="float-end flex flex-row gap-4 items-center">
      ...
      <a href="/register" class="bg-mainGreen h-min w-20 justify-center text-sm rounded-md flex font-thin text-white items-center">Sign Up</a>
  ...
  <main class="absolute z-10 top-10 left-0 w-full h-full justify-center">
    <div class="">
        <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto h-screen lg:py-0">
            <div class="w-full bg-white-1 rounded-2xl max-w-md">
                <div class="p-10">
                    ...
                    @error('email')
                    <div class="p-2 bg-red-300 rounded-lg w-full text-sm text-red-800 mt-3 italic">{{ $message }}</div>                        
                    @enderror
                    <form class="mt-4" action="/login" method="post">
                        ...
                        <div>
                            <label for="password" class="mt-6 ml-4 block mb-1 text-sm font-medium text-gray-900">Password</label>
                            <input type="password" name="password" id="password" placeholder="Enter your password" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-full focus:ring-primary-600 focus:border-primary-600 block w-full py-2.5 px-4 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500" required="">
                        </div>
                        <a href="" class="text-end font-bold text-xs hover:underline mt-1 text-mainGreen float-end">Forgot password?</a>
                        <button type="submit" class="mt-6 w-full text-white bg-mainGreen font-bold rounded-full text-lg px-5 py-2 text-center">Login</button>
                        <p class="mt-6 text-sm text-center font-light text-gray-500 dark:text-gray-400">
                            Don't have account? <a href="/register" class="font-medium text-primary-600 hover:underline">Create now</a>
                        </p>
                    </form>
                </div>
            </div>
